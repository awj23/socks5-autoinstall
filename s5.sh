...
if [[ ${OS} == Ubuntu ]]; then
    echo ""
    echo "***********************"
    echo "* 正在为 Ubuntu 安装依赖 *"
    echo "***********************"
    apt update -y
    apt install -y git unzip wget gcc g++ automake make libpam0g-dev libldap2-dev libsasl2-dev libssl-dev
fi
